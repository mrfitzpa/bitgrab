...
def _get_version_summary():
    """Get version summary of ``bitgrab``.
    ...
    -------
    summary : `str`
        Version summary of ``bitgrab``.
    """
    # Check versions of bitgrab and numpy.
    from . import _version
    if _version.version != version:
        raise ValueError("bitgrab version has changed since "
                         "installation/compilation")

    # Generate summary.
    summary = ("bitgrab {bitgrab_ver!s};\n"
               "git revision {git_rev!s} using\n"
               "python {python_ver!s}")
    
    summary = summary.format(bitgrab_ver=full_version,
